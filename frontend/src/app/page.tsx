...
const slides = [
  ...
  return (
    ...
    <motion.div
  className="mt-6 sm:mt-10 w-full grid grid-cols-2 gap-3 sm:flex sm:flex-row sm:justify-center sm:gap-4"
  initial={{ opacity: 0, y: 30 }}
  animate={{ opacity: 1, y: 0 }}
...
>
  {["Try SRE Lock Free", "Download Case Study", "Get a Quote"].map((label, i) => (
    <button
      key={i}
      className={`
        w-full px-4 py-2 text-sm rounded-full font-semibold text-white bg-[#F7941E] hover:bg-[#E8850D]
        ${i === 2 ? 'col-span-2 sm:col-span-1 sm:w-auto justify-self-center max-w-[200px]' : 'sm:w-auto'}
