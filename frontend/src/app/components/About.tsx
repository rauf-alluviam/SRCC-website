...
export default function AboutUs() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full py-10 sm:py-14 md:py-16 lg:py-20 bg-gradient-to-b from-gray-50 to-gray-100 overflow-hidden"
    >
      {/* Hero Section */}
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row items-center px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20 gap-6 sm:gap-8 md:gap-10 lg:gap-12">
        ...
        >
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-[#5A4A42] mb-4 sm:mb-5 md:mb-6 whitespace-nowrap">
            ABOUT <br />
            <span className="text-[#5A4A42]">SR&nbsp;Container&nbsp;Carriers</span>
          </h2>
      ...
      <AnimatePresence>
        {showVisionMission && isInView && (
          <motion.div
            ref={cardsRef}
            className="fixed top-16 sm:top-20 md:top-24 left-1/2 transform -translate-x-1/2 z-50 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6 lg:gap-8 w-[92%] sm:w-[85%] md:w-[90%] lg:w-full max-w-5xl px-3 sm:px-4 md:px-6 overflow-y-auto md:overflow-y-visible max-h-[80vh] md:max-h-none"
            initial="hidden"
            animate="visible"
            exit="exit"
